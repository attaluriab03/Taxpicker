...
export default function LoginForm() {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Redirect to the correct admin URL based on current hostname.
      // Full page navigation so the browser commits the session cookie
      // before the middleware runs — router.push() can race against the
      // cookie being set and get redirected back to login.
      const currentHost = window.location.hostname
      const isAdminSubdomain =
        currentHost.startsWith('admin.') && currentHost !== 'localhost'

      window.location.href = isAdminSubdomain ? `https://${currentHost}` : '/admin'
    } catch (err: any) {
